		...
		if (formData.password !== formData.confirmPassword) {
		  ...
		  // You may want to reset the password fields or handle this case differently
			...
			try {
				...
				const response = await fetch('http://localhost:4000/insert/user', {
				  method: 'POST',
				  headers: {
	  ...
	  const handleSubmit2 = async (e) => {
		...
		try {
			...
			const response = await fetch(
				`http://localhost:4000/select/user/${loginData.name}/${loginData.pass}`
			  );
			  const jsonData = await response.json();
			if (jsonData !==null) {
				setData(jsonData.nom,jsonData.id);
				onLogin(); 
			navigate('/');
	...
	<div className="main">  	
			...
			<div className="signup">
				<form className="form">
					...
					<input className="input" type="text" name="userName" value={formData.userName} placeholder="Name LogIn" onChange={handleChange} required=""/>
